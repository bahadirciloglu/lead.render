...
class LLMLogger:
    ...
    def log_response(self, response: str, model: str, usage: Dict[str, int], metadata: Dict[str, Any] = None) -> str:
        """
        LLM yanıtını logla
    ...
    def log_conversation(self, prompt_id: str, response_id: str, metadata: Dict[str, Any] = None) -> str:
        """
    ...
    def log_llm_interaction(self, prompt: str, response: str, model: str, usage: Dict[str, int], metadata: Dict[str, Any] = None) -> Dict[str, str]:
        """
    ...
    def get_logs(self, log_type: str = "conversations", limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for log_type in ["prompts", "responses", "conversations"]:
                ...
                if file_path.exists():
                    with open(file_path, "r", encoding="utf-8") as f:
                        lines = f.readlines()
                        stats[f"total_{log_type}"] = len([line for line in lines if line.strip()])
                else:

...
class ProjectManagementRepository:
    ...
    def get_all_weeks(self) -> List[Dict]:
        ...
        try:
            ...
            result = self.db.client.table('project_management').select('*').order('week_number').execute()
    ...
    def save_week(self, week_data: Dict) -> Optional[str]:
        ...
        try:
            ...
            existing_result = self.db.client.table('project_management').select('id').eq('week_number', week_number).execute()
            ...
            insert_data = {
                ...
                "week_name": week_name,
                "date_range": week_data.get("dateRange", "Week Date Range"),  # Boş string yerine default değer
            ...
            result = self.db.client.table('project_management').insert(insert_data).execute()
    ...
    def update_week(self, week_id: str, update_data: Dict) -> bool:
        ...
        try:
            ...
            result = self.db.client.table('project_management').update(update_fields).eq('id', week_id).execute()
    ...
    def delete_week(self, week_id: str) -> bool:
        """Hafta sil"""
        try:
            result = self.db.client.table('project_management').delete().eq('id', week_id).execute()
